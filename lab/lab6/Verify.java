...
public class Verify {
	...
	public void randomTrees( int _noNodes, int _nTrees ) {
		...
		for( int i = 0; i < _nTrees; ++i ) {
			// TODO #1 Create AVL and BST trees
			...
			do {
				...
				if( treeAVL.get ( randKey ) == null ) // Needs change !!!
					   // TODO #2 insert into AVL
					treeAVL.put(randKey,  null);
					// TODO  #3insert into BST
					treeBST.put(randKey, null);
					// next key
					++count;
			}
			while( count < _noNodes );
			updateCounters ( height(treeAVL), height(treeBST)); // TODO #4 Needs change !!!
		}
		return;
	} /* randomTrees */
	...
	public < T, V > int height( TreeMap <T, V>bTree ) {
		return bTree.height();
		//return 0; // TODO change to return the height of bTree
	} /* height */
